...
if($app) { 
    ...
    $app.uninstall() Write-Host "Searching for existing Sysmon...."
If (Get-WmiObject -Class Win32_Service -Filter "Name='Sysmon'") {
    Write-Host "Uninstalling Sysmon..."
    Start-Process -FilePath "Sysmon.exe" -Wait -ArgumentList "-u"
}
...
schtasks -erroraction 'silentlycontinue' /Delete /TN "UAT" /F | Out-Null
schtasks -erroraction 'silentlycontinue' /Delete /TN "UATupload" /F | Out-Null 
schtasks -erroraction 'silentlycontinue' /Delete /TN "DFPM" /F | Out-Null 
